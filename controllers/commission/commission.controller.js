...
module.exports.GetAllDay = async (req, res) => {
    try {
        ...
        console.log(err);
        return res.status(500).send({status: false, message: "มีบางอย่างผิดพลาด", err: "server side error"});
    }
};
...
module.exports.GetAllWeek = async (req, res) => {
    try {
        ...
        return res.status(200).send({status: true, message: "ดึงข้อมูลสำเร็จ", data: commission_week});
    } catch (err) {
        console.log(err);
        return res.status(500).send({status: false, message: "มีบางอย่างผิดพลาด", err: "server side error"});
    }
